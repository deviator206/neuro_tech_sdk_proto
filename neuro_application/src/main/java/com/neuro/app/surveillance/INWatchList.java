...
public final class INWatchList extends BasePanel implements ActionListener {
...
//								NotificationStatus.VISIBLE, timeStampIn);
	...
	private final NSurveillanceListener subjectDisappearedListener = new NSurveillanceListener() {
		...
		public void eventOccured(NSurveillanceEvent ev) {
			for (NSurveillanceEventDetails details : ev.getEventDetailsArray()) {
				...
				try {
					Timestamp timeStampIn = dbService
					...
					dbService.markAttendanceInHistory(CameraType.IN, unMatchedId, timeStampIn, timestamp);
					unknownID++;
	...
	private final ChangeListener runningChangeListener = new ChangeListener() {
		...
		public void stateChanged(ChangeEvent e) {
			NSurveillance surveillance = INSurveillanceTools.getInstance().getSurveillance();
			if (INSurveillanceTools.getInstance().getSurveillance().isRunning()) {
				currentFrameIndex = 0;
			} else {
	...
	public INWatchList(int percent) throws Throwable {
		super();
		...
		biometricClient.setDatabaseConnectionToOdbc("Dsn=neurotechnology;UID=root;PWD=passw0rd", "inimageStore");
	...
	private void updateCameras() {
		...
		for (NDevice device : INSurveillanceTools.getInstance().getSurveillance().getDeviceManager().getDevices()) {
	...
	private void showConnectTo() {
		...
		if (connectPanel.isResultOk()) {
			...
			newDevice = INSurveillanceTools.getInstance().getSurveillance().getDeviceManager()
			...
			if (comboBoxCameras.getSelectedItem() != newDevice) {
				if (newDevice != null) {
					INSurveillanceTools.getInstance().getSurveillance().getDeviceManager()
	...
	private NSurveillanceSource getSource() throws IOException {
		...
		if (rbCamera.isSelected()) {
			...
			camera = (NCamera) INSurveillanceTools.getInstance().connectDevice(file, false);
		} else {
...
//				if (mapping.getValue().get(2).equals("IN")) {
				...
				if (status == NBiometricStatus.OK) {
					...
					((DefaultTableModel) iTableResults.getModel()).addRow(new Object[] { id, 0, null, 0 });
				} else {
...
//				}
	...
	private void clearWatchSubjects() {
		...
		((DefaultTableModel) iTableResults.getModel()).setRowCount(0);
	}
	...
	private void incrementDetectedCount(String id, Date date) {
		DefaultTableModel model = (DefaultTableModel) iTableResults.getModel();
		int rowCount = iTableResults.getRowCount();
...
//				model.setValueAt((Integer) model.getValueAt(i, 1) + 1, i, 1);
	...
	protected void initGUI() {
		...
		panelSourceOuter.setBorder(BorderFactory.createTitledBorder("Inward Source"));
		...
		panelWatchList.setBorder(BorderFactory.createTitledBorder("Inward Watch list"));
		...
		iTableResults.getColumnModel().getColumn(1).setCellRenderer(leftRenderer);
		iTableResults.getColumnModel().getColumn(3).setCellRenderer(leftRenderer);

		iTableResults.setDefaultRenderer(Object.class, new ColorTableCellRenderer());

		scrollPaneResults.setViewportView(iTableResults);
		...
		tfVideo.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (fc.showOpenDialog(INWatchList.this) == JFileChooser.APPROVE_OPTION) {
	...
	protected void initTab() {
		...
		iTableResults.getColumnModel().getColumn(1).setMinWidth(-1);
		iTableResults.getColumnModel().getColumn(1).setWidth(-1);
		iTableResults.getColumnModel().getColumn(1).setMaxWidth(-1);
		iTableResults.getColumnModel().getColumn(1).setPreferredWidth(-1);

		// hiding the state column in watchList jTable -- TRUPTI

		iTableResults.getColumnModel().getColumn(3).setMinWidth(-1);
		iTableResults.getColumnModel().getColumn(3).setWidth(-1);
		iTableResults.getColumnModel().getColumn(3).setMaxWidth(-1);
		iTableResults.getColumnModel().getColumn(3).setPreferredWidth(-1);
	}
	...
	public void actionPerformed(ActionEvent ev) {
		try {
			if (ev.getSource().equals(btnStart)) {
				...
				if (device != null) {
					INSurveillanceTools.getInstance().getSurveillance().getDeviceManager().disconnectFromDevice(device);
				}
				updateCameras();
